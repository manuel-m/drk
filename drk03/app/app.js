...
var dbcsCodec = { _dbcs: _dbcs },
  ...
  encodings = createCommonjsModule(function(module, exports) {
    for (
      ...
      for (var enc in (module = modules[i]))
...
function first(stuff, done) {
  ...
  for (var cleanups = [], i = 0; i < stuff.length; i++) {
    ...
    for (var ee = arr[0], j = 1; j < arr.length; j++) {
      ...
      ee.on(event, fn), cleanups.push({ ee: ee, event: event, fn: fn });
    }
  }
  function callback() {
      ...
      return (
        ...
        void next(
          httpErrors(
            ...
            { charset: charset, type: 'charset.unsupported' }
          )
        )
      );
...
var has = Object.prototype.hasOwnProperty,
  ...
  lib$1 = { formats: formats, parse: parse$4, stringify: stringify_1 },
  ...
  (Route.prototype.dispatch = function(req, res, done) {
    ...
    'head' !== method || this.methods.head || (method = 'get'),
      ...
      (function next(err) {
        ...
        var layer = stack[idx++];
        return layer
          ? layer.method && layer.method !== method
            ? next(err)
            : void (err
...
function mapWithIndex(range, index) {
  return { start: range.start, end: range.end, index: index };
}
...
function parseAcceptEncoding(accept) {
  ...
  return (
    hasIdentity ||
      (accepts[j++] = { encoding: 'identity', q: minQuality, i: i }),
    (accepts.length = j),
    accepts
  );
}
...
function splitParameters(str) {
  for (
    ...
    quoteCount(parameters[j]) % 2 == 0
      ...
      : (parameters[j] += ';' + parameters[i]);
  for (parameters.length = j + 1, i = 0; i < parameters.length; i++)
